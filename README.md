...
$$ K(x,x') = <\phi(x),\phi(x')> $$
<br>
The weight vector $w$ is calculated implicity. A vector $\alpha$  counts the number of the performed corrections for every example $j$. The mathematical reformulation are referred to the original paper [1].
